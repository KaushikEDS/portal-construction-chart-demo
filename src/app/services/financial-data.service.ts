...
export class FinancialDataService {
    ...
    public getTestData(){
        ...
        for(let i = 0 ; i <= 50;i++) {
          for(let j = 1 ; j <= 2;j++){
            const minValue = i < 50 ? 0 : i -50;
            const high = this.getRandomInt(minValue,i)
            result.push(
                { high, low: high - 50, close:this.getRandomInt(minValue,i), open: this.getRandomInt(minValue,i) ,time:new Date(1965 + i, j, this.getRandomInt(0,29))  }
            )
          }
      
        }
        result.title = 'Testing'
        return result;
    }
